...
class WorksTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        \App\Models\ResourceQiniu::where('key','like','seeder/videos/%')->get()->each(function ($model,$index)use(&$video_ids){
            $video_ids[] = $model->resource->id;
        });

        //封面资源id
        $cover_ids = [];

        \App\Models\ResourceQiniu::where('key','like','seeder/cover/%')->limit(count($video_ids))->get()->each(function ($model,$index)use(&$cover_ids){
            $cover_ids[] = $model->resource->id;
        });
